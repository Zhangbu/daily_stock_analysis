...
- Agent 首条 user message 继续瘦身：预取的实时行情与筹码分布不再原样注入整段 JSON，而是改为关键字段摘要；上下文复用提示也改为短句形式，进一步减少每轮对话输入 token。
- Agent 聊天 prompt 新增 history 窗口限制：多轮对话仅回传最近 6 条历史消息进入模型，减少长会话下的重复上下文开销。
- Agent 历史分析上下文同步瘦身：`previous_analysis_summary` / `previous_strategy` 在聊天跟进场景下改为短摘要注入，不再原样拼接长 JSON。
...
- 🎯 **AI 归因分析**
  ...
  - 板块相对强度对比：识别个股是否"掉队"或"领涨"
  - 生成归因描述和操作建议
  - 新增 `src/ai_attribution.py` 模块
  - Agent 工具：`analyze_attribution`

- 📊 **概率化支撑压力位**
  - 关键价位识别：整数关口、前高/前低、均线位置、成交量密集区
  - 历史反弹率统计：触及次数、反弹次数、反弹概率
  - 强度评级：极强/强/中/弱/极弱 五档划分
  - 置信度评估：基于样本数量和概率一致性
  - 示例输出："30.0元整数关口支撑强度高（历史触及14次，反弹率72%）"
  - 新增 `src/probability_levels.py` 模块
  - Agent 工具：`get_probability_levels`

- 🐉 **龙虎榜席位深度分析**
  - 识别知名游资席位（呼家楼、拉萨天团、章盟主等），追踪主力资金动向
  - 席位净买卖统计与信号判断（bullish/bearish/neutral）
  - 机构专用席位识别与净买入统计
  - 板块资金集中度分析：识别主力资金正在集中的板块
  - 连续上榜追踪：股票连续多日出现在龙虎榜的追踪
  - 新增 `src/lhb_seat_analyzer.py` 模块

- 📊 **个股 vs 板块相对强度分析**
  - 自动识别个股所属行业/板块
  - 对比个股与板块的涨跌幅差异，识别"掉队"股票
  - 表现类型分类：领涨、跟随、背离、破位补跌
  - 板块概况统计：上涨/下跌家数、涨停/跌停数、领涨股
  - 操作建议生成：基于相对强度的交易建议
  - 新增 `src/relative_strength.py` 模块

- 🌡️ **市场温度计（情绪指数）**
  - 综合情绪指数（0-100）：恐慌 → 贪婪五档划分
  - 市场宽度统计：上涨/下跌/涨停/跌停家数
  - 北向资金流向：净流入/流出趋势判断
  - 赚钱效应监测：平均振幅、换手率
  - 仓位建议：基于情绪指数的仓位管理建议
  - 新增 `src/market_sentiment.py` 模块

- 🔬 **增强分析整合模块**
  - 统一 API 整合龙虎榜、相对强度、市场情绪三大模块
  - 综合信号生成：bullish/bearish/neutral + 置信度
  - 关键发现汇总：一站式查看所有分析亮点
  - 操作建议合并：多维度分析融合后的交易建议
  - 新增 `src/enhanced_analysis.py` 模块

- 💰 **赚钱效应精确监测**
  - 昨日涨停股今日表现追踪：成功率、平均收益率
  - 连板率统计：市场接力意愿指标
  - 赚钱效应指数（0-100）：极佳/良好/一般/较差/极差五档
  - 板块表现分析：涨停股分布与成功率
  - 风险提示生成：打板亏钱效应预警
  - 新增 `src/money_effect.py` 模块

- 🔄 **主线轮动图谱**
  - 热门板块识别：涨幅、涨停数、资金流入综合评分
  - 板块生命周期判断：萌芽期、发酵期、高潮期、衰退期
  - 板块强度评级：五星评级体系
  - 龙头股识别：板块领涨股、龙二龙三追踪
  - 操作建议生成：基于板块阶段的交易策略
  - 新增 `src/sector_rotation.py` 模块

- 🚨 **异动实时推送**
  - 放量突破检测：量比>=2且上涨的股票
  - 急涨急跌预警：涨幅>=5%或跌幅>=5%
  - 换手异常监测：换手率>=10%的股票
  - 龙虎榜大额交易：净买入/卖出>=5000万
  - 异动分级：重要/高/中优先级
  - 新增 `src/realtime_alert.py` 模块
