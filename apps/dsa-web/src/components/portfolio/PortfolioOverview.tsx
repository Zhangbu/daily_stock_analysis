...
/* ------------------------------------------------------------------ */
/*  Business logic — buildHoldingView                                 */
/* ------------------------------------------------------------------ */
  ...
  for (const seed of seeds) {
    ...
    if (!current) {
      grouped.set(key, {
        ...
        pnlPct: seed.avgCost > 0
          ? ((normalizeValue(seed.lastPrice) - normalizeValue(seed.avgCost)) / normalizeValue(seed.avgCost)) * 100
          : null,
        accountCount: 1,
    ...
    current.pnlPct = current.avgCost > 0
      ? ((current.lastPrice - current.avgCost) / current.avgCost) * 100
      : null;
  }
...
const OverviewStatCards: React.FC<OverviewStatCardsProps> = ({ snapshot, risk }) => {
  const holdings = useMemo(() => buildHoldingView(snapshot, risk), [snapshot, risk]);
...
const OverviewAlerts: React.FC<OverviewAlertsProps> = ({ risk, snapshot }) => {
  const hasAlerts =
...
const ActionBoard: React.FC<ActionBoardProps> = ({ holdings }) => {
  const focusHoldings = useMemo(() => holdings.slice(0, 5), [holdings]);
...
const RiskDistribution: React.FC<RiskDistributionProps> = ({ risk }) => {
    ...
    <div className="space-y-4">
      <div>
        ...
        <div className="space-y-2">
          {sectorHighlights.length > 0 ? (
            sectorHighlights.slice(0, 5).map((item) => (
              <div
                key={item.sector}
                className="flex items-center justify-between rounded-xl border border-subtle/80 bg-card/40 px-3 py-2 text-sm"
              >
                <div>
...
const HoldingsTable: React.FC<HoldingsTableProps> = ({ holdings }) => {
  if (holdings.length === 0) {
    return <p className="text-sm text-secondary-text">当前筛选条件下没有持仓数据。</p>;
  }
...
export const PortfolioOverview: React.FC<PortfolioOverviewProps> = ({ snapshot, risk }) => {
  const holdings = useMemo(() => buildHoldingView(snapshot, risk), [snapshot, risk]);

  return (
    <div className="space-y-6">
      <OverviewStatCards snapshot={snapshot} risk={risk} />
      <OverviewAlerts risk={risk} snapshot={snapshot} />
      <div className="grid gap-6 xl:grid-cols-[1.2fr,0.8fr]">
        <SectionCard
          title="优先关注"
          subtitle="Action Board"
          actions={holdings.length > 0 ? <Badge variant="info">{holdings.slice(0, 5).length} 项焦点</Badge> : null}
        >
          <ActionBoard holdings={holdings} />
        </SectionCard>
        <SectionCard title="风险分布" subtitle="Exposure">
          <RiskDistribution risk={risk} />
        </SectionCard>
      </div>
      <SectionCard
        title="持仓清单"
        subtitle="Holdings"
        actions={<Badge variant="default">{holdings.length} 只标的</Badge>}
      >
