...
> A 股单股分析已增强资金与板块结构判断：在原有技术面、筹码和新闻基础上，会额外纳入主力资金流、龙虎榜异动、所属板块与板块强弱，用于提升对“是否有主线支撑”的判断质量。

> A 股单股分析还会补充日内结构信号，例如高开低走、低开高走、收盘位置、上下影线，以及距离涨跌停价的强弱特征，帮助更准确地区分“承接转强”和“冲高回落”。

> 这些 A 股专属信号现在也会以结构化标签形式保留在历史详情和回测结果中，便于后续做筛选、复盘和因子验证。
...
同一套策略页能力现在也可以复用于更大的美股股票池，例如：

- `nasdaq100`：Nasdaq-100 成分股画像
- 同样支持 `mag7_ma_pullback`、`mag7_breakout`、`mag7_ma_cross`
- Web 工作台可直接进入 `Nasdaq-100` 页面
