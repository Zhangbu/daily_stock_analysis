...
class IntelCoordinatorTestCase(unittest.TestCase):
    ...
    def test_collect_comprehensive_intel_uses_query_scoped_cache(self):
        response = Mock(success=True, results=[Mock()], query="q")
        self.search_service.search_comprehensive_intel.return_value = {"latest_news": response}
        self.search_service.format_intel_report.return_value = "formatted-intel"

        first = self.coordinator.collect_comprehensive_intel(
            code="600519",
            stock_name="贵州茅台",
            query_id="q1",
            max_searches=3,
        )
        second = self.coordinator.collect_comprehensive_intel(
            code="600519",
            stock_name="贵州茅台",
            query_id="q1",
            max_searches=3,
        ...
        self.assertEqual(first, "formatted-intel")
        self.assertEqual(second, "formatted-intel")
        self.search_service.search_comprehensive_intel.assert_called_once()
        self.persistence.save_news_intel.assert_called_once()

    def test_persist_latest_news_for_agent_uses_query_scoped_cache(self):
        response = Mock(success=True, results=[Mock()], query="latest")
        self.search_service.search_stock_news.return_value = response
        ...
        self.search_service.search_stock_news.assert_called_once()
        self.persistence.save_news_intel.assert_called_once()
